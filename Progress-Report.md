# SmartShop — Progress Report

Phase-by-phase implementation notes. For application overview, setup, and deployment, see [README.md](README.md).
...
Phase 8 adds inventory-aware retrieval, evaluation tooling, Docker packaging, and Hugging Face Spaces deployment for **`soumitkundu/smartshop`**.

### Deployment decisions

| Decision | Choice |
|----------|--------|
| HF Space | `soumitkundu/smartshop` |
| Architecture | Single container — FastAPI (`8781`) + Chainlit (`7860`) |
| Bundled data | `products.json`, `text_index.json`, `chroma_db/` built during Docker image build |
| Cold start | Whisper + CLIP pre-downloaded at image build time |
| GitHub → HF | Connect repo in Space settings; HF rebuilds on each push |

### Implemented (code)

Added / updated:

- `evaluation/test_queries.json` — catalog + guardrail evaluation cases
- `evaluation/ragas_eval.py` — proxy metrics (+ optional `--ragas`)
- Inventory filtering via `EXCLUDE_OUT_OF_STOCK=true` (default)
- Enhanced `GET /api/health` — text index, ChromaDB, LLM key checks
- `Dockerfile` — preload models + build catalog artifacts
- `deployment/preload_models.sh` — Whisper + CLIP cache at build time
- `deployment/build_catalog.sh` — format Kaggle → text index → ChromaDB
- `deployment/start.sh` — one-container startup script
- `deployment/README_deploy.md` — GitHub → Hugging Face guide
- `README.md` — HF Space frontmatter (`sdk: docker`, `app_port: 7860`)

### Inventory filtering

When `EXCLUDE_OUT_OF_STOCK=true`, retrieval over-fetches and returns only products with `inventory_quantity > 0`.
...
Optional RAGAS: `py evaluation/ragas_eval.py --ragas` (requires `pip install ragas datasets`)

### Deploy (GitHub → Hugging Face)

1. Create Space **soumitkundu/smartshop** (SDK: **Docker**, Public)
2. Space **Settings → Repository → Connect to GitHub** → select repo + `main`
3. Add secrets: `GROQ_API_KEY`, `SHOPIFY_STORE_DOMAIN`, etc.
4. Push to GitHub — HF rebuilds automatically

Full guide: `deployment/README_deploy.md`
...
|-------|----------------|--------|
| Evaluation suite | `py evaluation/ragas_eval.py` | PASS |
| Health endpoint | `GET /api/health` | Enhanced checks |
| Docker build scripts | `deployment/build_catalog.sh` | Catalog baked into image |
| HF Space | `https://huggingface.co/spaces/soumitkundu/smartshop` | Connect GitHub + secrets |
